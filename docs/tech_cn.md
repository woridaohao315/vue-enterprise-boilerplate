...
我们的JavaScript由Babel使用[`@vue/babel-preset-app`](https://github.com/vuejs/vue-cli/tree/dev/packages/%40vue/babel-preset-app)作为基本配置进行编译。您可以在.babelrc.js中更新此配置。

如果你不熟悉相关功能，例如 `const`, `let`, 以及 `=>` (箭头函数), 花点时间阅读一下Babel's ES2015 guide中的以下功能:
...
单独阅读这些部分将使您掌握99％的Babel代码。如果您对Promises尚不满意，那你最好接着阅读一下这个[比较好的介绍](https://developers.google.com/web/fundamentals/getting-started/primers/promises)。
...
本项目采用Vue的CLI的[modern mode](https://cli.vuejs.org/guide/browser-compatibility.html#modern-mode)，创建了两个包：一个现代化的包用于那些支持[ES modules](https://jakearchibald.com/2017/es-modules-in-browsers/)的现代浏览器，以及一个传统包用于那些不支持es模块的旧版本浏览器。

对于每个包，对于任何`JavaScript polyfills`功能，您使用包括由`browserslist`中的`package.json`定义了基于的包和支持的浏览器。

### Vue

由于Vue是我们应用程序的重要组成部分，因此我强烈建议每个人至少通读[Vue要点指南](https://vuejs.org/v2/guide/)。
